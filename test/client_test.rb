...
\e[1m[TRB PRO] view trace (ClientTest::Create) at \e[22mhttps://ide.trailblazer.to/#{trace_id})
  ...
  it "web/cli trace settings" do
    ...
    Trailblazer::Pro::Session.trace_guards = Trailblazer::Pro::Trace::Decision.new(
      [
        ->(activity, *) { activity == ClientTest::Create ? [Trailblazer::Pro::Trace::Wtf, {render_wtf: true}] : false }, # false means "call super"
        ->(activity, ctx) { activity == Trailblazer::Pro::Debugger::Push && ctx[:data_to_store][:fields][:activity_name][:stringValue] == ClientTest::Create ? [Trailblazer::Developer::Wtf, {}] : false }, # false means "call super"
      ]
    )
...
|-- \e[32mmodel\e[0m
`-- End.success
\e[1m[TRB PRO] view trace (ClientTest::Create) at \e[22mhttps://ide.trailblazer.to/#{trace_id}) # Create is on CLI and web.
  end
end
